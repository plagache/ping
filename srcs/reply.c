...
void print_information_from_received_message(t_icmp_packet_reply *echo_reply)
{
	...
	if (echo_reply->icmp_header.un.echo.id == g_ping->program_id)
	{
		fprintf(stdout, "%lu bytes from %s: icmp_seq=%d ttl=%d time=%.3f ms\n", g_ping->bytes_received - sizeof(struct iphdr), source_address_string, echo_reply->icmp_header.un.echo.sequence, echo_reply->ip_header.ttl, (double)get_current_time_diff((struct timeval *)echo_reply->data) / 1000);
	}
}
