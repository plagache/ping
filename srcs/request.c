...
void sending_icmp_echo_request(int file_descriptor){
    ...
    g_ping->bytes_send = sendto(file_descriptor, &g_ping->packet, sizeof(g_ping->packet), 0, (struct sockaddr *) &g_ping->ipv4_address, sizeof(address_data));

    if (g_ping->bytes_send <= 0){
        fprintf (stdout, "bytes_send with sendto = %i\n", g_ping->bytes_send);
